      ...
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        <form>
          ...
          <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
            ...
            <input className='bg-transparent outline-none' type="password"
              placeholder="Password" required />
          </div>
          <p className='mb-4 text-end text-sm text-indigo-500 cursor-pointer'>Forget Password?</p>
        ...
        {state === 'Sign Up' ?  (
          ...
          <span onClick={() => setState('Login')} className='text-indigo-500 cursor-pointer underline'>Login</span></p>
        ) : (
          <p className='text-gray-400 text-center text-xs mt-4'>Don't have an account? {' '}
          <span onClick={() => setState('Sign Up')} className='text-indigo-500 cursor-pointer underline'>Sign Up</span></p>
        )}
      </div>
    </div>
  )
