...
const Navbar = () => {
    ...
    const sendVerificationOtp = async ()=>{
      try{
        ...
        const {data} = await axios.post(backendUrl + '/api/auth/send-verifiy-otp')
        if(data.success){
  ...
  return (
    ...
    return (
        <button onClick={handleLogout} className='flex items-center gap-2 border border-gray-500
        rounded-full px-6 py-2 text-gray-800 hover:bg-gray-100
        transition-all'>Logout <img src={assets.arrow_icon} alt="" /></button>
    )
